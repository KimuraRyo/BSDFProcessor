...
void CharacteristicDockWidget::addReflectanceItems()
{
    ...
    for (int phIndex = 0; phIndex < data_->getNumInPhi(); ++phIndex) {
        if (data_->getNumInPhi() > 1) {
            ...
            parentItem->setText(0, "Incoming azimuthal angle: " + QString::number(lb::toDegree(inPhi)) + u8"Åã");
        }
        ...
        for (int thIndex = 0; thIndex < data_->getNumInTheta(); ++thIndex) {
            ...
            item->setText(0, QString::number(lb::toDegree(inTheta)) + u8"Åã");
...
void CharacteristicDockWidget::add8DegreeReflectanceItems()
{
    ...
    switch (data_->getDataType()) {
        case lb::BRDF_DATA:
        case lb::SPECULAR_REFLECTANCE_DATA:
            reflectanceItem->setText(0, u8"Reflectance at 8Åã");
            reflectanceItem->setToolTip(0, u8"8Åã:di (CIE 15, JIS Z 8722)");
            break;
        case lb::BTDF_DATA:
        case lb::SPECULAR_TRANSMITTANCE_DATA:
            reflectanceItem->setText(0, u8"Transmittance at 0Åã");
            reflectanceItem->setToolTip(0, u8"0Åã:di (CIE 15, JIS Z 8722)");
            break;
        default:
            return;
    }
